...
export type MultiDropdownProps = {
  ...
  options: Option[];
  value: Option[];
  onChange: (value: Option[]) => void; // eslint-disable-line no-unused-vars
  disabled?: boolean;
  getTitle: (value: Option[]) => string; // eslint-disable-line no-unused-vars
};

const MultiDropdown: React.FC<MultiDropdownProps> = observer(
  ({ className, options, value, onChange, disabled = false, getTitle }) => {
    ...
    const displayPlaceholder = useMemo(() => {
      return (isOpen && filter === "") || (!isOpen && value.length === 0)
        ? getTitle(value)
        : "";
    }, [isOpen, filter, value, getTitle]);
    const inputClassName = useMemo(() => {
      ...
      if (isOpen && value.length > 0) {
        cn += `${styles["multi-dropdown__input--open-selected"]}`;
      } else if (!isOpen && value.length > 0) {
        cn += `${styles["multi-dropdown__input--closed-selected"]}`;
      }
      return cn;
    }, [isOpen, value]);
              ...
              <div
                ...
                className={[
                  ...
                  selectedKeys.has(opt.key)
                    ? styles["multi-dropdown__option--selected"]
                    : "",
                ]
                  .filter(Boolean)
                  .join(" ")}
